...
RSpec.describe Jsonapi::QueryBuilder::Paginator::Kaminari do
  describe '#paginate' do
    ...
    let(:collection_class) do
      Class.new(Kaminari::PaginatableArray) do
    ...
    let(:collection) { instance_double collection_class }
    let(:paged_collection) do
      instance_double(collection_class, current_page: 2,
                                        limit_value: 20,
                                        total_count: 35,
                                        total_pages: 2,
                                        next_page: nil,
    ...
    before do
      allow(collection).to receive_messages(page: collection, per: collection, padding: paged_collection)
    end
