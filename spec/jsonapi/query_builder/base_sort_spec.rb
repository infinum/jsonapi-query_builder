...
RSpec.describe Jsonapi::QueryBuilder::BaseSort do
  ...
  it 'defaults to ascending sort direction' do
    expect(FakeSort.new(instance_double(ActiveRecord::Relation))).to have_attributes(direction: :asc)
  end

  context 'with required interface methods' do
    it 'raises an error for results method' do
      expect { FakeSort.new(instance_double(ActiveRecord::Relation), :desc).results }.to raise_error(
